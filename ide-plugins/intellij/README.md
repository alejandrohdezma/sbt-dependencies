...
- **Resolved versions**: `*` and `{{variable}}` dependencies show their concrete version as ghost text (with a `(stale)` marker after edits), read from the `.sbt-resolutions` dump the sbt plugin writes on load; hovers reveal the pinning BOM or variable. `Alt+Enter` materializes a `*` into its resolved version, and hardcoded or `{{variable}}` versions a BOM manages are flagged with a rewrite to `*` — applied to that line or, when more than one qualifies, to all of them at once.
- **Note hints**: pinned or intransitive entries without a `note` are flagged with an `Add note` intention; single-line object entries with a single annotation fold into `"dependency" // note` (or `// only for Scala 2.13` / `// overrides` for a `scala-filter` or `overrides = true`), expanding while the caret is inside them; entries with several annotations stay expanded so none of them is hidden.
...
- **Source links**: `⌘`-click on a dependency opens its source repository (usually GitHub), read from the `<scm>` URL of its POM in the local Coursier cache; dependencies without a cached POM open mvnrepository.com instead.
