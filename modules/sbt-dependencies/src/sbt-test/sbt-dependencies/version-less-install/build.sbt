...
assertTest := {
  val content = IO.read(baseDirectory.value / "project" / "dependencies.yaml")

  // Verify cats-effect is present with a version (format: cats-effect:X.Y.Z)
  assert(content.contains("cats-effect:"), s"cats-effect should be in dependencies.yaml, content: $content")
